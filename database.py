...
class Database:
    ...
    def check_freefire_latam_api_availability(self, amount_value):
        """Verificar si la API de Free Fire Latam tiene PINs disponibles SIN consumirlos"""
        import requests
        import os
        ...
        # Validar valores específicos de Free Fire Latam (1-9)
        if amount_value < 1 or amount_value > 9:
            print(f"[FREEFIRE LATAM CHECK] Valor {amount_value} inválido")
            return False

        # Parámetros para verificar disponibilidad (usando action especial si existe)
        params = {
            'action': 'consulta',  # Acción de consulta en lugar de recarga
            'usuario': provider_user,
            'clave': provider_password,
        ...
        try:
            ...
            # Procesar respuesta para verificar disponibilidad
            try:
                import json
                json_response = json.loads(response_data)
                return self._check_freefire_latam_availability_response(json_response)
            except json.JSONDecodeError:
                # Si no es JSON válido, intentar buscar indicadores en texto plano
                return self._check_freefire_latam_text_availability(response_data)

        except Exception as e:
            print(f"[FREEFIRE LATAM CHECK] Error verificando disponibilidad: {e}")
            return False

    def _check_freefire_latam_availability_response(self, json_response):
        """Verificar disponibilidad desde respuesta JSON"""
        alert_status = json_response.get('ALERTA') or json_response.get('alerta', '').upper()
        
        # Si el status es verde/green, hay disponibilidad
        if alert_status == 'VERDE' or alert_status == 'GREEN':
            return True
            
        # Verificar mensajes que indiquen disponibilidad
        mensaje = json_response.get('MENSAJE', '').lower()
        if 'sin stock' in mensaje or 'no disponible' in mensaje or 'agotado' in mensaje:
            return False
            
        # Si hay un PIN en la respuesta, hay disponibilidad
        pin_code = json_response.get('PIN') or json_response.get('pin')
        if pin_code and len(pin_code.strip()) >= 4:
            return True
            
        return False

    def _check_freefire_latam_text_availability(self, response_data):
        """Verificar disponibilidad desde respuesta de texto plano"""
        response_lower = response_data.lower()
        
        # Indicadores de no disponibilidad
        ...
        # Por defecto, asumir no disponible si no hay indicadores claros
        return False
